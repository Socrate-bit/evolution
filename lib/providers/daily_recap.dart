      ...
      onCreate: (db, version) {
        return db.execute('''
          CREATE TABLE RecapDay (
            ...
            wellBeing INTEGER,
            energy INTEGER,
            driveMotivation INTEGER,
            stress INTEGER,
            focusMentalClarity INTEGER,
            intelligenceMentalPower INTEGER,
            frustrations INTEGER,
            satisfaction INTEGER,
            selfEsteemProudness INTEGER,
            lookingForwardToWakeUpTomorrow INTEGER,
            recap TEXT,
            improvements TEXT,
            gratefulness TEXT,
            proudness TEXT,
  ...
  Future<void> loadRecapDays() async {
    ...
    final List<RecapDay> loadedData = data.map((row) {
      return RecapDay(
        ...
        sleepQuality: row['sleepQuality'] as int? ?? 0,
        wellBeing: row['wellBeing'] as int? ?? 0,
        energy: row['energy'] as int? ?? 0,
        driveMotivation: row['driveMotivation'] as int? ?? 0,
        stress: row['stress'] as int? ?? 0,
        focusMentalClarity: row['focusMentalClarity'] as int? ?? 0,
        intelligenceMentalPower: row['intelligenceMentalPower'] as int? ?? 0,
        frustrations: row['frustrations'] as int? ?? 0,
        satisfaction: row['satisfaction'] as int? ?? 0,
        selfEsteemProudness: row['selfEsteemProudness'] as int? ?? 0,
        lookingForwardToWakeUpTomorrow: row['lookingForwardToWakeUpTomorrow'] as int? ?? 0,
